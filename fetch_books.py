...
def process_repos(repos):
    books = []
    # Match the standard description pattern:
    # "Epub source for the Standard Ebooks edition of TITLE, by AUTHOR"
    # or "Epub source for the Standard Ebooks edition of TITLE, by AUTHOR. Translated by TRANSLATORS"
    pattern = re.compile(r"^Epub source for the Standard Ebooks edition of (.*?),\s+by\s+(.*?)(?:\.\s+Translated\s+by\s+(.*))?$")
